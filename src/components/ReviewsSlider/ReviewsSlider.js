...
import ReviewsSliderList from "./ReviewsSliderList/ReviewsSliderList";
...
const ReviewsTitle = styled.h2`
  ...
  font-size: 2rem;
`;
const ReviewsIconBox = styled.div`
  width: 40px;
  height: 40px;
  border-radius: 50%;
  background: #ffe99c;
...
const ReviewsSliderBox = styled.div`
  position: relative;
`;

const ReviewsSliderBtn = styled.button`
  position: absolute;
  top: 50%;
  ...
  cursor: pointer;
`;
const ReviewsSliderLeftBtn = styled(ReviewsSliderBtn)`
  left: -45px;
`;
const ReviewsSliderRightBtn = styled(ReviewsSliderBtn)`
  right: -45px;
`;
...
function ReviewsSlider() {
  ...
  return (
    <ReviewsSection ref={title} className="gs_reveal">
      <ReviewsHeader>
        <ReviewsIconBox>
          <svg
            width="24"
      ...
      <ReviewsSliderBox>
        <ReviewsSliderList reviews={reviews} ref={reviewsList} />
      </ReviewsSliderBox>
    </ReviewsSection>
  );
}
